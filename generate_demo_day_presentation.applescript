...
set deckName to "KMG_HR_AI_Command_Center_Demo_Day"
...
tell application "Keynote"
	activate
	set docRef to make new document with properties {document theme:theme "Простая черная"}
	tell docRef
		set width to 1920
		set height to 1080
		delete slide 1
	...
	my add_bullet_slide(docRef, "Заголовок и пункты", "1. Проблема бизнеса", "SMART сам по себе не гарантирует качество цели." & return & "Цели часто не связаны с KPI, ВНД и задачами руководителя." & return & "Руководители не видят зрелость goal set по подразделениям." & return & "Performance management превращается в формальность, а не в инструмент управления.")
	
	my add_bullet_slide(docRef, "Заголовок и пункты", "2. Что мы построили", "Employee workspace: роль, проекты, KPI, каскад от руководителя и health набора целей." & return & "Goal Studio: SMART, strategic alignment, duplicate risk и улучшенная формулировка." & return & "AI generation: 3-5 целей на основе ВНД, KPI и контекста сотрудника." & return & "Team radar: зрелость целеполагания по подразделениям и кластеры рисков.")
	
	my add_bullet_slide(docRef, "Заголовок и пункты", "3. Как работает демо", "1. Показываем dashboard зрелости целеполагания." & return & "2. Выбираем сотрудника и собираем контекст не в вакууме, а в контуре бизнеса." & return & "3. Проверяем слабую цель и видим SMART-разбор, связку с ВНД и риски." & return & "4. Генерируем новый набор целей и возвращаем лучшую формулировку обратно в Goal Studio.")
	
	my add_bullet_slide(docRef, "Заголовок и пункты", "4. Почему решение сильное", "Это не просто SMART-checker, а command center для качества целеполагания." & return & "Hybrid architecture: deterministic scoring + retrieval + optional external LLM." & return & "Решение объяснимо: каждая цель подкрепляется ВНД, KPI и контекстом роли." & return & "Есть ценность и для сотрудника, и для руководителя, и для HR/exec уровня.")
	...
	my add_bullet_slide(docRef, "Заголовок и пункты", "6. Внедрение и next steps", "Подключение к боевой HR-системе вместо CSV." & return & "Историческая калибровка достижимости по фактическим результатам." & return & "Feedback loop от руководителей и HR BP." & return & "Расширение с уровня одной цели до управления полным квартальным goal set.")
